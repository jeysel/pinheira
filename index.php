<?php get_header(); ?>
	...
	if (is_category()) {
   		...
   		echo '<h1 class="listhead">';
   		_e("<strong> resultados da pesquisa</strong>", "notesblog");
   		echo '</h1>';
   	}
   	?>
	...
	<?php else : ?>
		...
		<?php if (is_search()) { ?>
			<div class="post single">
				<h2><?php _e('Nenhum resultado!','notesblog');?></h2>
				<p><?php _e('A sua pesquisa não retornou nenhum resultado. Por que não tentar novamente? Ajuste sua consulta e tente novamente.','notesblog');?></p>
				<h3><?php _e('Algo para ler?','notesblog');?></h3>
				<p><?php _e('Estas são as atualizações mais recentes:','notesblog');?></p>
